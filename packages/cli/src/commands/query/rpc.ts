  ...
  rpcCmd
    ...
    .action(async (options) => {
      try {
        ...
        const request = createQueryRequest({ domain: "rpc", op: "health-timeline", params: { since: options.since }, limit });
        const result = await engine.execute(request);
        if (options.json) {
          const { serializeQueryResult } = await import("@hardkas/query");
          console.log(serializeQueryResult(result));
        } else { printRpcHealthTimeline(result); }
  ...
  rpcCmd
    ...
    .action(async (options) => {
      try {
        ...
        const engine = new QueryEngine({ artifactDir: process.cwd() });
        const limit = parseInt(options.limit, 10) || 100;
        const request = createQueryRequest({ domain: "rpc", op: "degradations", params: { since: options.since }, limit });
        const result = await engine.execute(request);
        if (options.json) {
          const { serializeQueryResult } = await import("@hardkas/query");
          console.log(serializeQueryResult(result));
        } else { printRpcDegradations(result); }
      } catch (e) { handleError(e); process.exitCode = 1; }
    });
  ...
  rpcCmd
    ...
    .action(async (txId, options) => {
      try {
        ...
        const request = createQueryRequest({ domain: "rpc", op: "correlate", params: { txId }, explain });
        const result = await engine.execute(request);
        if (options.json) {
          const { serializeQueryResult } = await import("@hardkas/query");
          console.log(serializeQueryResult(result));
        } else { printRpcCorrelation(result); }
      } catch (e) { handleError(e); process.exitCode = 1; }
    });
}
